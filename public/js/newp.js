...
for (var i = 1; i < 2; i++) {
    ...
    $('.paymenttable').append(`<tr  class=" m-0 row text-center">
      ...
      <td   class="col-md-1 p-0"><input class="form-control" name="dctype" id="dctype-${i}" type="text" maxlength="2" oninput="javascript:oninputdctype(event);" />

      <td  class="col-md-5 p-0"><input  name="accounttype-${i}" onkeypress="accounttype(event)" id="accounttype-${i}" type="text" class="form-control" disabled /><div style= "max-height:150px; margin:auto; overflow-y:scroll; position:relative; z-index:99; width:100% !important;"><ul class="list-group" style="max-height:150px; width:100%;" id="accountlist-${i}" class="d-none" disabled></ul></div></td>
      <td  class="col-md-2 p-0"><input onkeypress="createnew(event)" oninput="javascript:checktotaldc(event);" name="debit" id="debit-${i}"  type="number" min="0" class="form-control" disabled /></td>
      <td   class="col-md-2 p-0"><input onkeypress="createnew(event)" oninput="javascript:checktotaldc(event);" name="credit" id="credit-${i}"  type="text" pattern="[०-९]*" class="form-control" disabled/></td>
    </tr>`)
}
...
function createnew(e)
{
    ...
    if($(`#accounttype-${i}`).length === 0 && e.keyCode===
        13)
  {
    $('.paymenttable').append(`<tr  class=" m-0 extraadded  row text-center">
...
// $("[id^=accounttype]").on('keydown', function(event) {
    ...
    var accountnamedetails= $(".accountnamedetails a");
    for(i=0;i<accountnamedetails.length;i++)
    {
          if (($(accountnamedetails).eq(i).text()).indexOf(a) !== -1 ) {

            $(accountnamedetails).eq(i).addClass('bg-info');
            $(accountnamedetails).eq(i).addClass('selected');


 handle($(accountnamedetails).eq(i));
...
else
{
            ...
            if($(accountnamedetails).eq(i).hasClass('bg-info'))
{
            $(accountnamedetails).eq(i).removeClass('bg-info');
        }


            if($(accountnamedetails).eq(i).hasClass('selected'))
{
            $(accountnamedetails).eq(i).removeClass('selected');
        }
